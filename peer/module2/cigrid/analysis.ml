...
exception InvalidTreeException of string
...
let rec env_get_type lst ident =
		match lst with
		| [] -> raise (InvalidTreeException "Type not found in env.")
		| Symbol(r, t) :: rs ->
			if r = ident then t
...
let rec get_type t type_env =
	...
	| TIdent(r) -> 
		...
		else raise (InvalidTreeException "Undeclared type used.")
...
let rec traverse_expr e type_env var_env =
	...
	| EVar(r) -> 
		...
		else raise (InvalidTreeException "EVar undeclared.")
	| EInt(i) 	-> ()
	| EChar(c) 	-> ()
	| EString(s) 	-> ()
	| EBinOp(b, e1, e2) -> 
	...
	| ECall(r, e_lst) ->
		if env_contains type_env r then 
			...
			if env_contains var_env r then
				let (_) = traverse_expr_list e_lst type_env var_env in 
				()
			else raise (InvalidTreeException "ECall undefined and undeclared.")

	| ENew(t, e1) -> ()

	| EArrayAccess(r, e1, r_opt) -> 
		let (_) = traverse_expr e1 type_env var_env in
		()
...
let rec traverse_stmt s type_env var_env =
	...
	| SVarDef(t, r, e) -> 
		...
		if (env_contains var_env r) = false then 
			...
			(type_env, var_env2)
		else raise (InvalidTreeException "SVarDef already exists.")

	| SVarAssign(r, e) -> 
		if (env_contains var_env r) = true then
			let (_) = traverse_expr e type_env var_env in
			(type_env, var_env)
		else raise (InvalidTreeException "SVarAssign to undeclared variable.")
				...
				let bt = iterate_types r_type type_env in (
					...
					| Struct(_, sym_lst) -> 
						if env_contains sym_lst member then 
							let (_) = traverse_expr e2 type_env var_env in
							(type_env, var_env)
						else raise (InvalidTreeException "Struct member does not exist.")
					| _ -> raise (InvalidTreeException "base type is not a struct.")
				)
			| None -> 
				let (_) = traverse_expr e2 type_env var_env in 
				(type_env, var_env)
			)	
		else raise (InvalidTreeException "SArrayAssign to undeclared variable.")
	...
	| SDelete(r) ->
		...
		else raise (InvalidTreeException "SDelete undeclared variable.")
...
let rec traverse_param p type_env var_env =
	...
	| Param(t,r) ->
		...
		else raise (InvalidTreeException "param already exists.")
	| _ -> 
		raise (InvalidTreeException "Invalid param.")
...
let rec traverse_global g type_env var_env =
	...
	| GFuncDef(t, r, p_lst, s) -> 
		...
		if (env_contains var_env r) = false then
			...
			if (env_contains type_env r) then
				...
				else raise (InvalidTreeException "Function definition does not match declaration.")
			else (type_env, var_env2)
		else raise (InvalidTreeException "Function defined already exist.")
	...
	| GFuncDecl(t, r, p_lst) -> 
		...
		if (env_contains type_env r) = false then
			...
			if (env_contains var_env r) then 
				...
				else raise (InvalidTreeException "Function declaration does not match definition.")
			else (type_env2, var_env)
		else raise (InvalidTreeException "Function already declared exist.")
	...
	| GVarDef(t,r,e) -> 
		...
		if (env_contains var_env r) = false then
			...
			if (env_contains type_env r) then
				let ot = env_get_type type_env r in
				if ot = pt then (type_env, var_env2)
				else raise (InvalidTreeException "Global Variable definition does not match declaration.")
			else 
			(type_env, var_env2)
		else raise (InvalidTreeException "Global Variable defined already exist.")
	...
	| GVarDecl(t,r) -> 
		...
		if (env_contains type_env r) = false then
			...
			if (env_contains var_env r) then 
				let ot = env_get_type var_env r in
				if ot = pt then (type_env2, var_env)
				else raise (InvalidTreeException "Global Variable declaration does not match definition.")
			else (type_env2, var_env)
		else raise (InvalidTreeException "Global Variable already declared exist.")
	...
	| GStruct(r, p_lst) ->
		...
		if (env_contains type_env r) = false then 
			...
			(type_env3, var_env)
		else raise (InvalidTreeException "Struct defined already exist.")
...
let nameAnalysis tree =
	try 
		...
		| _ -> 
			raise (InvalidTreeException "No program given.")
	with
	| InvalidTreeException(msg) 	-> Printf.fprintf stderr "%s\n" msg; exit 2 
	(* Always do both name analysis and type checking but only return
	 * type errors if type flag is true
   	 *)
